...
> **Audious** est une plateforme **d’auto‑hébergement de bibliothèque audio** minimaliste et rapide.  
...
> Audious diffuse uniquement les fichiers que vous lui fournissez : il vous appartient de n’importer que des contenus pour lesquels vous disposez des droits ou d’une autorisation d’usage conforme à la loi et aux CGU des plateformes concernées.  
> Certains scripts d’exemple (notamment ceux s’appuyant sur des outils externes comme `yt-dlp`) sont fournis uniquement pour un usage strictement personnel et ne font pas partie du cœur officiel du projet.  
...
3. [📻 Radio (Liquidsoap + Icecast)](#-radio-liquidsoap--icecast)
4. [🧰 Installation rapide](#-installation-rapide)
5. [🗄️ Base de données](#️-base-de-données)
6. [⚙️ Configuration `.env`](#️-configuration-env)
7. [🗃️ Permissions & dossiers système](#-permissions--dossiers-système)
8. [📧 Email & vérification de compte](#-email--vérification-de-compte)
9. [🎵 Import audio & gestion des tags](#-import-audio--gestion-des-tags)
10. [🔗 Intégration avec outils externes (yt-dlp, etc.)](#-intégration-avec-outils-externes-yt-dlp-etc)
11. [🧱 Structure simplifiée](#-structure-simplifiée)
12. [🧩 Commandes CLI intégrées](#-commandes-cli-intégrées)
13. [🧪 Sécurité & modes d’exécution](#-sécurité--modes-dexécution)
14. [⚠️ Avertissement légal (contenus tiers)](#️-avertissement-légal-contenus-tiers)
15. [🔮 Roadmap](#-roadmap)
16. [🧾 Licence & Droits](#-licence--droits)
...
| 📡 Infra    | Apache/Nginx • **Icecast** (radio) • **Liquidsoap** (moteur de stream) • Bash • CLI PHP |
...
Audious peut être utilisé comme une **radio** : un stream audio continu, alimenté par tout ou partie de la base.

### Rôle des briques

- **Icecast** : serveur de diffusion (les auditeurs se connectent au stream).
- **Liquidsoap** : moteur qui enchaîne des fichiers audio (playlist / règles / fallback) et “pousse” vers Icecast.
- **Audious (DB + fichiers)** : bibliothèque et métadonnées (titres, artistes, tags, etc.).

### Historique

Audious peut afficher un historique de lecture Icecast via une table dédiée (ex. `icecast_history`) et des liaisons avec les `songid` Audious.

### Now playing

> ⚠️ À ce jour, Audious **ne récupère pas encore automatiquement le now playing** (titre en cours) depuis Icecast/Liquidsoap pour l’afficher dans l’UI (WIP).  
> Le stream fonctionne, l’historique fonctionne, mais l’état “morceau courant” côté UI reste à finaliser.
...
| `MAIL_FROM_ADDRESS`  | Adresse “from” (ex. `audious@audious.dev`) |
...
1. Placer des fichiers dans le staging :
...
# (ou court) php scan/import.php -s /home/ubuntu/audious/scan/staging --move --verbose
```

> ⚠️ **Important :** l’utilisation d’outils comme `yt-dlp` peut être soumise aux CGU des plateformes et au droit d’auteur. Vous êtes entièrement responsable de vérifier la conformité de vos usages.
...
> Certains scripts additionnels (d’exemples ou d’outillage) peuvent exister ailleurs, mais ne sont pas nécessaires au fonctionnement de base d’Audious.
...
| `php bin/audious-cli.php files:dedupe --path=/home/ubuntu/music [...]` | Détection de doublons de fichiers |
...
- Le projet ne fournit aucune garantie quant à la conformité de vos usages avec les CGU de plateformes tierces et le droit d’auteur.

En utilisant Audious, **vous êtes seul responsable** de la provenance des fichiers importés et de leur diffusion éventuelle (privé vs public).
